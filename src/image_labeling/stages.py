...
def run_ingest(ctx: RunContext) -> None:
        ...
        manifest_rows.append(
            {
                ...
                "content_hash": sha256_file(file_path) if file_path and file_path.exists() else None,
            }
        )
    ...
    record_data_artifact(ctx, "manifest", "ingest", manifest_path, len(manifest_rows))
    ...
    ctx.write_stage_outputs("ingest", summary, sample=manifest_rows[: ctx.config.runtime.sample_limit], errors=errors)


def run_quality_check(ctx: RunContext) -> None:
    manifest_rows = read_jsonl(ctx.path("data", "manifest.jsonl"))
    ...
    for row in manifest_rows:
        result = _quality_for_row(ctx, row)
    ...
    record_data_artifact(ctx, "quality_results", "quality_check", quality_path, len(quality_rows))
    ...
    ctx.write_stage_outputs("quality_check", summary, sample=quality_rows[: ctx.config.runtime.sample_limit], errors=errors)


def run_thumbnail(ctx: RunContext) -> None:
    quality_rows = read_jsonl(ctx.path("data", "quality_results.jsonl"))
    ...
    for row in quality_rows:
        if not row.get("local_path"):
            errors.append(_stage_error(row, "non_local_image", "thumbnail currently requires local image paths"))
            continue
        ...
        try:
            ...
            errors.append(_stage_error(row, exc.__class__.__name__, str(exc)))

    thumb_index = ctx.path("data", "thumbnails.jsonl")
    write_jsonl(thumb_index, thumb_rows)
    record_data_artifact(ctx, "thumbnails", "thumbnail", thumb_index, len(thumb_rows))
    summary = {
        "stage": "thumbnail",
        "input_count": len(quality_rows),
        "success_count": len(thumb_rows),
        "failed_count": len(errors),
        "thumbnail_dir": ctx.uri(thumb_dir),
    }
    ctx.write_stage_outputs("thumbnail", summary, sample=thumb_rows[: ctx.config.runtime.sample_limit], errors=errors)
...
def run_auto_decision(ctx: RunContext) -> None:
    quality_rows = {row["image_id"]: row for row in read_jsonl(ctx.path("data", "quality_results.jsonl"))}
...
def _load_input_rows(ctx: RunContext) -> tuple[list[dict[str, Any]], list[dict[str, Any]]]:
    ...
    if input_config.type in {"local_dir", "object_prefix"}:
        ...
        rows = [
            {"uri": ensure_local_uri(path), "metadata": {"relative_path": str(path.relative_to(root))}}
...
def _quality_for_row(ctx: RunContext, row: dict[str, Any]) -> dict[str, Any]:
    ...
    if file_size < ctx.config.quality.min_file_size_bytes:
        actions.append("needs_review")
        reasons.append("file_too_small")
    if ctx.config.quality.max_file_size_bytes and file_size > ctx.config.quality.max_file_size_bytes:
    ...
    try:
        ...
        with Image.open(path) as image:
            ...
            if fmt not in ctx.config.quality.allowed_formats:
                actions.append("drop")
                reasons.append("unsupported_format")
            if width < ctx.config.quality.min_width or height < ctx.config.quality.min_height:
                actions.append("drop")
                reasons.append("image_too_small")
            if width * height < ctx.config.quality.min_pixels:
                actions.append("drop")
                reasons.append("too_few_pixels")
            aspect = max(width / max(height, 1), height / max(width, 1))
            if aspect > ctx.config.quality.max_aspect_ratio:
                actions.append("needs_review")
                reasons.append("aspect_ratio_extreme")
            if ctx.config.quality.blur.enabled:
                blur_score = _blur_score(image)
                result["blur_score"] = blur_score
                if blur_score < ctx.config.quality.blur.edge_variance_threshold:
                    actions.append(ctx.config.quality.blur.action)
                    reasons.append("blurry")
            if ctx.config.quality.low_information.enabled:
                stddev = _channel_stddev(image)
                result["channel_stddev"] = stddev
                if stddev < ctx.config.quality.low_information.channel_stddev_threshold:
                    actions.append(ctx.config.quality.low_information.action)
